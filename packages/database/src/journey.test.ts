...
describe('seeded customer-to-mechanic journey', () => {
  it('creates, accepts, completes, and invoices a job', async () => {
    ...
    const appointment = (await db.query<{ id: string }>("INSERT INTO appointments (quote_id, customer_id, mechanic_id, starts_at, ends_at, timezone, status) VALUES ($1,$2,$3,now()+interval '2 hours',now()+interval '3 hours','America/New_York','scheduled') RETURNING id", [quote.id, ids.customer, ids.mechanic]))[0];
