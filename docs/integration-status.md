...
| Repair Room messages/findings | `/jobs/{id}/messages`, `/jobs/{id}/findings` contract planned only | implemented (dedicated sections: empty-state thread + disabled composer for messages, placeholder for findings) | mocked-honest-pending | verified via browser + DOM check that the composer's textarea/button are genuinely `disabled`, not just styled inert. CR-004 filed for both endpoints. |
| Tap Your Trouble intake | `/service-requests`, `/uploads` contract planned only | implemented (`/intake/:category`: real 4-step wizard — vehicle from `GET /vehicles`, symptom chips, client-side photo preview via object URLs, review) | mocked-honest-pending (submission) | verified via browser click-through of all 4 steps plus DOM check that the final Submit button is genuinely `disabled`. CR-005 filed for both endpoints. |
